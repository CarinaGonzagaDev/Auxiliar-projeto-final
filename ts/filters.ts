...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('click', (event) => {
        if (!genreButton?.contains(event.target as Node) && !genreContainer?.contains(event.target as Node)) {
    ...
    if (filtersForm) {
        filtersForm.addEventListener('submit', (event) => {
            ...
            document.querySelectorAll<HTMLInputElement>('input[name="genre"]:checked').forEach(checkbox => {
                selectedGenres.push(checkbox.value);
            });
            
            const filters = {
                name: (document.getElementById('search-name') as HTMLInputElement).value,
                year: (yearSelect as HTMLSelectElement).value,
                // LÓGICA ATUALIZADA AQUI:
                // Quando for implementar a busca, lembre-se: se o valor for "10", a busca é por == 10.
                // Se for outro valor (ex: "7"), a busca é por >= 7.
                rating: (ratingSelect as HTMLSelectElement).value,
                sort: (document.getElementById('filter-sort') as HTMLSelectElement).value,
                genres: selectedGenres,
                audio: (document.getElementById('filter-audio') as HTMLSelectElement)?.value,
                country: (document.getElementById('filter-country') as HTMLSelectElement)?.value,
            };
