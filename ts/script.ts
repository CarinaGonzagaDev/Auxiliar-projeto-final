...
// --- DADOS E LÓGICA DO CARROSSEL EM UM SÓ LUGAR ---
...
interface Work {
    ...
    episode?: number;
    chapters?: number;
    pageUrl: string; // Link para a página da obra
}
...
class Carousel {
    ...
    constructor(trackId: string, prevBtnId: string, nextBtnId: string, data: Work[]) {
        ...
        if (!track || !prevBtn || !nextBtn) { console.error(`Falha ao inicializar carrossel: ${trackId}`); return; }
    ...
    private setupEventListeners(): void { this.nextButton.addEventListener('click', () => this.goToNext()); this.prevButton.addEventListener('click', () => this.goToPrev()); window.addEventListener('resize', () => this.updatePosition()); }
    private updatePosition(): void { if (!this.trackElement) return; const items = this.trackElement.querySelectorAll('.carousel-item'); if (items.length === 0) return; const item = items[0] as HTMLElement; const itemStyle = window.getComputedStyle(item); const itemMarginRight = parseFloat(itemStyle.marginRight); const itemWidth = item.offsetWidth + itemMarginRight; const offset = -this.currentIndex * itemWidth; this.trackElement.style.transform = `translateX(${offset}px)`; }
    private goToNext(): void { const itemsPerView = this.getItemsPerView(); if (!itemsPerView) return; const maxIndex = this.data.length - itemsPerView; if (this.currentIndex < maxIndex) { this.currentIndex++; } else { this.currentIndex = 0; } this.updatePosition(); }
    private goToPrev(): void { const itemsPerView = this.getItemsPerView(); if (!itemsPerView) return; if (this.currentIndex > 0) { this.currentIndex--; } else { this.currentIndex = this.data.length - itemsPerView; } this.updatePosition(); }
    private getItemsPerView(): number | null { if (!this.trackElement) return null; const viewport = this.trackElement.parentElement as HTMLElement; if (!viewport) return 1; const viewportWidth = viewport.offsetWidth; const item = this.trackElement.querySelector('.carousel-item') as HTMLElement; if (!item) return 1; const itemStyle = window.getComputedStyle(item); const itemMargin = parseFloat(itemStyle.marginLeft) + parseFloat(itemStyle.marginRight); const itemWidth = item.offsetWidth + itemMargin; return Math.max(1, Math.floor(viewportWidth / itemWidth)); }
}
